...
当前项目已改为仅分析美股（US tickers），并使用 Grok（xAI）API。

实现方式：
- 通过 OpenAI 兼容 SDK 调用（`openai` Python 包）
- 默认 base_url 为 `https://api.x.ai/v1`
- 使用环境变量 `XAI_API_KEY`（或 `OPENAI_API_KEY`）
"""
...
class GeminiAnalyzer:
    """
    LLM 分析器（保留历史类名 `GeminiAnalyzer` 以兼容现有入口）

    职责：
    1. 调用 OpenAI 兼容 API（默认 xAI Grok）进行股票分析
    ...
    SYSTEM_PROMPT = """你是一位专注于趋势交易的美股（US equities/ETFs）投资分析师，负责生成专业的【决策仪表盘】分析报告。
...
5. **风险优先级**：舆情中的风险点要醒目标出"""
    ...
    def __init__(self, api_key: Optional[str] = None):
        """
        初始化 AI 分析器（仅 OpenAI 兼容 API；默认 xAI Grok）
        ...
        self._current_model_name = config.llm_model

        key_valid = self._api_key and not self._api_key.startswith('your_') and len(self._api_key) > 10
        if not key_valid:
            logger.warning("未配置有效的 LLM API Key（XAI_API_KEY 或 OPENAI_API_KEY），AI 分析功能将不可用")
            return
        ...
        try:
            ...
            if config.llm_base_url and config.llm_base_url.startswith('http'):
                client_kwargs["base_url"] = config.llm_base_url

            self._openai_client = OpenAI(**client_kwargs)
            self._use_openai = True
            logger.info(f"LLM 初始化成功 (base_url: {config.llm_base_url}, model: {self._current_model_name})")
        except ImportError:
    ...
    def _call_api_with_retry(self, prompt: str, generation_config: dict) -> str:
        ...
        if not self._openai_client:
            raise RuntimeError("LLM 未初始化：请配置 XAI_API_KEY/OPENAI_API_KEY")
    ...
    def analyze(
        ...
        if not self.is_available():
            return AnalysisResult(
                ...
                risk_warning='请配置 XAI_API_KEY（或 OPENAI_API_KEY）后重试',
                success=False,
        ...
        try:
            ...
            # 获取模型名称
            model_name = getattr(self, '_current_model_name', None) or config.llm_model
            ...
            # 记录响应信息
            logger.info(f"[LLM返回] LLM API 响应成功, 耗时 {elapsed:.2f}s, 响应长度 {len(response_text)} 字符")
